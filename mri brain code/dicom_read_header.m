...
% end
%  datasets = datasets("/Users/breylonriley/Downloads/Utilities/DRR/M46343/MR.M46343.Image 0.dcm")

f = dir("MR*.dcm").name;
data = char(pwd + "/" + string(f)); 
datasets = dicom_folder_info(data); 
% datasets = datasets(data)
% 
% filenameLength = zeros(length(datasets)); 
% 
% for d = 1:length(datasets)
%     filenameLength(d) = datasets(d).Filenames;
% end 
% 
% [~, idx] = max(filenameLength);
